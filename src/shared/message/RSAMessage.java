...
public class RSAMessage implements Serializable {
	...
	public RSAMessage(byte[] in, boolean align){
		...
		if (align) {
			...
			if ((bytesToAddInv = (byte) (this.message.length % (int) readChunkSize)) != 0) {
				...
				ByteBuffer temp = (ByteBuffer) ByteBuffer
						.allocate(this.message.length + (readChunkSize - bytesToAddInv)).put(in).put(zeros);
	...
	/**
	 * Constructor. Uses default byte-aligning.
	 * 
	 * @param in
	 *          String to wrap (converted to byte array)
	 */
	...
	/**
	 * Construct blank RSAMessage (empty string)
	 */
	public RSAMessage(){
		this("");
	}
	...
	/**
	 * Encrypt byte array with given public key. Non-mutable method: does not alter this.
	 * 
	 * @param key
	...
	public RSAMessage encryptMessage(KeyFile key){
		...
		byte readChunkSizeLocal = RSAMessage.readChunkSize;
		...
		while (count < this.message.length) {
			byte[] temp = Arrays.copyOfRange(this.message, count, count + readChunkSizeLocal);
			count += readChunkSizeLocal;
			...
			output.put((byte) randWrite.length);
			bytesWritten++;
			output.put(randWrite);
			bytesWritten += randWrite.length;

			// encrypt (xor with random number. Send both result and random number)
			BigInteger data = new BigInteger(1, temp).xor(randNum).modPow(key.getKey(),
					key.getGroupSize());
			byte[] dataBytes = data.toByteArray();
			byte[] toWrite = ByteBuffer.allocate(dataBytes.length).put(data.toByteArray()).array();
			output.put((byte) toWrite.length);
			bytesWritten++;
			output.put(toWrite);
	...
	/**
	 * Decrypt byte array with given public key. Non-mutable method: does not alter this.
	 * 
	 * @param key
	...
	public RSAMessage decryptMessage(KeyFile key){
		...
		while (input.hasRemaining()) {
			...
			input.get(dataIn);
			BigInteger dataInBI = new BigInteger(dataIn).modPow(key.getKey(), key.getGroupSize()).xor(
					randInBI);
			...
			if ((sizeCheckTemp = dataInBI.toByteArray()).length != readChunkSize) {
				...
				if (sizeCheckTemp.length < readChunkSize) {
					...
					byte[] newData = ByteBuffer.allocate(readChunkSize).put(sizeCheckTemp).put(numZero)
							.array();
